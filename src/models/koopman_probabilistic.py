...
class KoopmanProb(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.num_fourier_modes = (
            kwargs["num_fourier_modes"] if "num_fourier_modes" in kwargs else 0
        )
    ...
    def find_fourier_omegas(self, xt, tt=None):
        ...
        """
        hard_coded_omegas = (
            2 * np.pi / torch.tensor(self.hard_code)
        ...
        if self.num_fourier_modes > 0:
            ...
            while num_found < self.num_fourier_modes:
                amax = np.argpartition(-adj_xt_ft[: len(xt_ft) // 2], i)[
                    i
                ]  # ith biggest freq
    ...
    def sgd(
        ...
        batches = idxs[: T // batch_size * batch_size].reshape(
    ...
    def predict(self, t, x0):
        ...
        k = torch.cat(
            [torch.cos(ts_ * o), torch.sin(ts_ * o)], -1
        )  # , ts_ / self.max_t], -1)
...
class CoordinateKoopmanProb(KoopmanProb):
    ...
    def sgd(
        ...
        for i in range(self.model_obj.num_freq[0]):
            opt = optim.SGD(
                self.model_obj.networks_mu[i].parameters(),
                lr=lr_theta,
        ...
        for i in range(self.model_obj.num_freq[1]):
            opt = optim.SGD(
                self.model_obj.networks_sigma[i].parameters(),
                lr=lr_theta,
                weight_decay=weight_decay,
            )
            opts.append(opt)
        ...
        for i in range(len(batches)):
            ...
            l1_norm = sum(
                p.abs().sum() for p in amps_params_mu
            )  # + sum(p.abs().sum() for p in amps_params_sig)
            if self.loss_weights is not None:
                weighted_losses = batch_losses * self.loss_weights[batches[i]]
