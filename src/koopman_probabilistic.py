...
class KoopmanProb(nn.Module):
    ...
    def sgd(self, xt, tt=None, weight_decay=0, verbose=False, lr_theta=1e-5, lr_omega=1e-5, training_mask=None):
        ...
        for i in range(len(batches)):
            ...
            k = torch.cat([torch.cos(wt), torch.sin(wt)], -1) #, ts_ / self.max_t], -1)
    ...
    def predict(self, T):
        ...
        k = torch.cat([torch.cos(ts_ * o), torch.sin(ts_ * o)], -1) #, ts_ / self.max_t], -1)
...
class CoordinateKoopmanProb(KoopmanProb):
    def __init__(self, model_obj, sample_num=12, seed=None, l1_coef=0, l2_coef=0, **kwargs):
        super(CoordinateKoopmanProb, self).__init__(model_obj, sample_num=12, seed=None, **kwargs)
    ...
    def sgd(self, xt, tt=None, weight_decay=0, verbose=False, lr_theta=1e-5, lr_omega=1e-5, training_mask=None):
        ...
        opt = optim.SGD(self.model_obj.parameters(), lr=lr_theta, weight_decay=weight_decay)
        opts = []
        for i in range(self.model_obj.num_freq[0]):
            opt = optim.SGD(self.model_obj.networks_mu[i].parameters(), lr=lr_theta, weight_decay=weight_decay)
            opts.append(opt)
        for i in range(self.model_obj.num_freq[1]):
            opt = optim.SGD(self.model_obj.networks_sigma[i].parameters(), lr=lr_theta, weight_decay=weight_decay)
            opts.append(opt)
        opt_mlp_mu = optim.SGD(self.model_obj.mlp_mu.parameters(), lr=lr_theta, weight_decay=self.l2_coef)
        opt_mlp_sigma = optim.SGD(self.model_obj.mlp_sigma.parameters(), lr=lr_theta, weight_decay=self.l2_coef)
        opt_omega = optim.SGD([omega], lr=lr_omega / T)
        ...
        for i in range(len(batches)):
            ...
            for opt in opts:
                opt.zero_grad()
            opt_mlp_mu.zero_grad()
            opt_mlp_sigma.zero_grad()
            opt_omega.zero_grad()
            ...
            l1_norm = sum(p.abs().sum() for p in amps_params_mu)# + sum(p.abs().sum() for p in amps_params_sig)
            if self.loss_weights is not None:
                weighted_losses = batch_losses * self.loss_weights[batches[i]]
                loss = torch.mean(weighted_losses) + self.l1_coef * l1_norm
            else:
                loss = torch.mean(batch_losses) + self.l1_coef * l1_norm
