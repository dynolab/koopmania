...
class NormalNLL(ModelObject):
    ...
    def get_modes_mu(self, w):
        # print(self.param_idxs, 'param idxs')
        # print(w.shape)
        w_mu = w
        y1 = nn.Tanh()(self.l1_mu(w_mu))
        y2 = nn.Tanh()(self.l2_mu(y1))
...
class MultiNormalNLL(ModelObject):
    ...
    def get_modes(self, x):
        ...
        for i in range(self.num_freq[0]):
            y.append(self.networks_mu[i].get_modes_mu(torch.index_select(x, -2, torch.tensor([i]))))
        mu = torch.cat(y, -2)

        z = []
        for i in range(self.num_freq[1]):
            z.append(
                self.networks_sigma[i].get_modes_sig(torch.index_select(x, -2, torch.tensor([self.num_freq[0] + i]))))
        sig = torch.cat(z, -2)
        return mu.flatten(-2), sig.flatten(-2)

    def get_amplitudes(self):
        return self.mlp_mu.state_dict()['weight'].flatten(), self.mlp_sigma.state_dict()['weight'].flatten()
    ...
    def decode(self, x):
        ...
        z = []
        for i in range(self.num_freq[1]):
            z.append(self.networks_sigma[i].get_modes_sig(torch.index_select(x, -2, torch.tensor([self.num_freq[0] + i]))))
        sig = torch.cat(z, -2)
