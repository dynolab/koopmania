...
functions = {"sine": gen_sine, "sawtooth": gen_sawtooth, "squares": gen_squares, "unit impulse": gen_unit_impulse}
...
#                              device='cpu')
model_k = koopman(fully_connected_mse(x_dim=1, num_freqs=num_freqs, n=512), 5, device='cpu')
...
print("MAPE Fourier:", MAPE(time_series[int(T * train_ratio):], pred_f.flatten()[int(T * train_ratio):]))
print("MAPE Koopman:", MAPE(time_series[int(T * train_ratio):], pred_k.flatten()[int(T * train_ratio):]))
